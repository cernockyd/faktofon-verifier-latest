...
export default function TopicCombobox({
  ...
  labels: LabelItem[];
  setLabels: React.Dispatch<React.SetStateAction<LabelItem[]>>;
  setSelected: React.Dispatch<React.SetStateAction<LabelItem[]>>;
  selected: LabelItem[];
  topicsKey: number;
}) {
    ...
    <React.Fragment>
      ...
      >
        <div className="w-full flex flex-col gap-1">
          <label className="text-sm leading-5 text-neutral-600" htmlFor={id}>
            Témata
          </label>
          <Combobox.Chips
              ...
              {(value: LabelItem[]) => (
                <React.Fragment>
                  ...
                  <Combobox.Input
                    ...
                    id={id}
                    placeholder={value.length > 0 ? "" : "např. EU"}
                    className="min-w-12 flex-1 h-8 rounded-md border-0 bg-transparent pl-2 text-base text-neutral-900 outline-none"
      ...
      <Dialog.Root open={openDialog} onOpenChange={setOpenDialog}>
        <Dialog.Portal>
          <Dialog.Backdrop className="fixed inset-0 min-h-dvh bg-neutral-700 opacity-20 transition-opacity dark:opacity-70 data-[starting-style]:opacity-0 data-[ending-style]:opacity-0 supports-[-webkit-touch-callout:none]:absolute" />
          <Dialog.Popup
            className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 mt-[-2rem] w-[24rem] max-w-[calc(100vw-3rem)] rounded-2xl bg-[canvas] p-6 text-neutral-900 outline-1 outline-neutral-200 transition-all data-[starting-style]:opacity-0 data-[starting-style]:scale-90 data-[ending-style]:opacity-0 data-[ending-style]:scale-90 dark:-outline-offset-1 dark:outline-neutral-300"
          ...
          >
            ...
            <Dialog.Description className="mb-4 text-base leading-6 text-neutral-600">
              Přidá nové téma do seznamu témat.
            </Dialog.Description>
            <form onSubmit={handleCreateSubmit}>
              <input
                ref={createInputRef}
                className="w-full h-10 rounded-md border border-neutral-200 bg-[canvas] text-neutral-900 px-2.5 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-neutral-400"
                placeholder="Label name"
                defaultValue={pendingQueryRef.current}
              />
              <div className="mt-4 flex justify-end gap-4">
                <Dialog.Close className="flex h-10 items-center justify-center rounded-md border border-neutral-200 bg-neutral-50 px-3.5 text-base font-medium text-neutral-900 select-none hover:bg-neutral-100 focus-visible:-outline-offset-1 focus-visible:outline-neutral-400 active:bg-neutral-100">
                  Zrušit
                </Dialog.Close>
                <button
                  type="submit"
                  className="flex h-10 items-center justify-center rounded-md border border-neutral-200 bg-neutral-50 px-3.5 text-base font-medium text-neutral-900 select-none hover:bg-neutral-100 focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-neutral-400 active:bg-neutral-100"
                >
                  Vytvořit
                </button>
              </div>
